...
export function printPaletteCatalog(colors: ColorData[], paletteName: string, modeName: string): void {
  ...
  let contrastRows = '';
  colors.forEach((c1) => {
    contrastRows += `<tr><td style="font-weight: 600; padding: 6px;">${c1.displayName}</td>`;
    colors.forEach((c2) => {
      if (c1.id === c2.id) {
        contrastRows += `<td style="text-align: center; color: #aaa; background: #fafafa; font-size: 10px;">-</td>`;
      } else {
  ...
  const swatchesHtml = colors.map(color => {
    ...
    return `
      <div style="display: flex; align-items: center; border-bottom: 1px solid #e5e5e5; padding: 12px 0; page-break-inside: avoid;">
        <div style="width: 80px; height: 60px; background-color: ${color.hex}; border: 1px solid #ddd; border-radius: 2px; margin-right: 20px; box-shadow: inset 0 0 2px rgba(0,0,0,0.1);"></div>
        <div style="flex: 1;">
          <div style="font-size: 15px; font-weight: 700; color: #111;">${color.displayName}</div>
          <div style="font-size: 11px; color: #666; margin-top: 2px;">ROLE: ${color.role.toUpperCase()} | ${color.temperature.toUpperCase()} TONE</div>
        </div>
  ...
  printContainer.innerHTML = `
    ...
    <div style="border-bottom: 2px solid #111; padding-bottom: 12px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: flex-end;">
      <div>
        ...
        <p style="font-size: 11px; color: #666; margin: 4px 0 0 0; letter-spacing: 1px; font-weight: 600;">QUIET FUTURE COLOR STUDIO • SWATCH CATALOG</p>
      </div>
      <div style="text-align: right;">
        <div style="font-size: 11px; font-weight: 700; color: #111;">DATE: ${dateStr.toUpperCase()}</div>
        <div style="font-size: 11px; color: #666; margin-top: 2px;">MODE: ${modeName.toUpperCase()}</div>
      </div>
    </div>
    
    <div style="margin-bottom: 30px;">
      <h2 style="font-size: 12px; font-weight: 700; color: #666; border-bottom: 1px solid #111; padding-bottom: 4px; text-transform: uppercase; margin-bottom: 10px;">Swatches</h2>
      ${swatchesHtml}
    </div>
    
    <div style="page-break-inside: avoid; margin-top: 30px;">
      <h2 style="font-size: 12px; font-weight: 700; color: #666; border-bottom: 1px solid #111; padding-bottom: 4px; text-transform: uppercase; margin-bottom: 15px;">Contrast &amp; Readability Matrix</h2>
      <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
        <thead>
          <tr style="border-bottom: 1px solid #ddd; background-color: #f9f9f9;">
            <th style="text-align: left; padding: 8px; font-size: 11px;">Color Name</th>
            ${colors.map(c => `<th style="padding: 8px; font-size: 10px; text-align: center; width: 80px;">${c.displayName.split(' ')[0]}</th>`).join('')}
          </tr>
        </thead>
        <tbody>
          ${contrastRows}
      ...
      <p style="font-size: 9px; color: #666; margin-top: 10px; line-height: 1.4;">
        * WCAG Contrast Ratio (target &ge; 4.5:1 for body text, &ge; 3.0:1 for large text).<br/>
        * APCA Lc score (target absolute score &ge; 75 for body reading, &ge; 45 for headers/buttons). Positive score represents Dark on Light text; negative represents Light on Dark text.
      </p>
    </div>
    ...
    <div style="position: fixed; bottom: 0; left: 0; right: 0; border-top: 1px solid #ddd; padding-top: 8px; display: flex; justify-content: space-between; font-size: 9px; color: #999;">
      <div>PRECISION SPACE OKLCH • GENERATED WITH CULORI ENGINE</div>
      <div>RAMS-MEAD RETRO FUTURIST SYSTEM</div>
    </div>
  `;
